...
describe("PinService", () => {
    ...
    beforeEach(() => {
        ...
        activityRepository = new ActivityRepository(dataSourceMock, activityServiceMock);
        ...
        webhookHandler = new WebhookHandler(dataSourceMock, configServiceMock, httpServiceMock);
        pinataService = new PinataService(configServiceMock) as jest.Mocked<PinataService>;
    ...
    describe("pinJSON", () => {
        it("should handle API timeout", async () => {
            ...
            jest.spyOn(pinService['metadataChanges'], 'removeEditMetadata').mockImplementationOnce(() => {
                throw { code: "ETIMEOUT" };
            });
            jest.spyOn(metadataChangesService, 'findWiki' as keyof MetadataChangesService).mockImplementationOnce(() => {
                throw { code: "ETIMEOUT" }
            })
    ...
    it('should throw error if API timeout occurs', async () => {
        const body = '{"title": "Example Wiki", "content": "This is an example wiki"}';
        jest.spyOn(pinService, 'pinJSON').mockRejectedValue({ code: 'ETIMEOUT' });
  
        await expect(pinService.pinJSON(body)).rejects.toEqual({ code: 'ETIMEOUT' });
});
        });
    });